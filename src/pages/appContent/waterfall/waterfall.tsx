...
const randomData = (len: number, type = 'width') => {
  ...
  for (let i = 0; i < len; i++) {
    res.push({
      ...
      backgroundColor: `rgba(${randomNum(0, 255)}, ${randomNum(
        0,
        255
      )}, ${randomNum(0, 255)}, 0.5)`,
      styles: {},
...
const getMaxHeight = (map: IDataModel[][]) => {
  const heightArr = map.map(
    (col) => (col[col.length - 1].top || 0) + col[col.length - 1].height
  );
...
function Waterfall() {
  const btnOption = [
    {
      text: '刷新数据',
      key: 'refresh',
      onclick: () => setData(randomData(DATA_NUM, equalType)),
    },
  ...
  useEffect(() => {
    if (flowContent.current) {
      ...
      // 等宽  根据容器宽度获取列数和间距
      const columnNum = Math.floor(
        flowContent.current.offsetWidth / data[0].width
      );
      const spacing =
        (flowContent.current.offsetWidth % data[0].width) / (columnNum - 1);
  ...
  const getFlowItem = (data: IDataModel[], type: 'width' | 'height') => {
    if (type === 'width') {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        const colSpacing =
          (clientWidth - row.reduce((total, next) => total + next.width, 0)) /
          (rowLen - 1);
    ...
    <div className="waterfall-container">
      <div className="flow-options fontWhite mb20 pb10">
        {btnOption.map((v, index) => (
          <span
            key={index}
            className={`pointer ${v.key === equalType ? 'active' : ''}`}
            onClick={v.onclick}
          >
            {v.text}
          </span>
