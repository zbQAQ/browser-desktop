...
const reducer = (
  state: IAircraftWarContext,
  action: IAnyAction<AIRCARFT_WAR_ACTION_TYPE>
) => {
  const {
    gameBoundary: { up, right, down, left },
    playerW,
    playerH,
    playerX,
    playerSpeed,
  ...
  switch (action.type) {
    ...
    case AIRCARFT_WAR_ACTION_TYPE.PLAYER_MOVE_LEFT:
      return {
        ...state,
        playerX: playerX - playerSpeed < left ? left : playerX - playerSpeed,
      };
    case AIRCARFT_WAR_ACTION_TYPE.PLAYER_MOVE_RIGHT:
      return {
        ...state,
        playerX:
          playerX + playerSpeed + playerW > right
            ? right - playerW
            : playerX + playerSpeed,
      };
    case AIRCARFT_WAR_ACTION_TYPE.PLAYER_MOVE_UP:
      return {
        ...state,
        playerY: playerY - playerSpeed < up ? up : playerY - playerSpeed,
      };
    case AIRCARFT_WAR_ACTION_TYPE.PLAYER_MOVE_DOWN:
      return {
        ...state,
        playerY:
          playerY + playerSpeed + playerH > down
            ? down - playerH
            : playerY + playerSpeed,
      };
    ...
    case AIRCARFT_WAR_ACTION_TYPE.BULLET_CLEAN:
      return bulletQueue.length > 0
        ? { ...state, bulletQueue: bulletQueue.filter((b) => !b.isDestory) }
        : state;
...
function spawnEnemy(state: IAircraftWarContext, difficulty: ENEMY_DIFFICULTY) {
  ...
  const { x, y } =
    ENEMY_SPAWN_POINTS[randomNum(0, ENEMY_SPAWN_POINTS.length - 1)];
  return {
    ...state,
    enemyQueue: [
      ...enemyQueue,
...
function enemyRondomMove(state: IAircraftWarContext, id: string) {
  ...
  const moveLeft = () =>
    (enemy.x = enemy.x - enemy.speed < left ? left : enemy.x - enemy.speed);
  const moveUp = () =>
    (enemy.y = enemy.y - enemy.speed < up ? up : enemy.y - enemy.speed);
  const moveRight = () =>
    (enemy.x =
      enemy.x + enemy.speed + enemy.w > right
        ? right - enemy.w
        : enemy.x + enemy.speed);
  const obstacleMoveDown = () =>
    (enemy.y = enemy.y + enemy.speed > down ? down : enemy.y + enemy.speed);
  const normalMoveDown = () =>
    (enemy.y =
      enemy.y + enemy.speed + enemy.h > down
        ? down - enemy.h
        : enemy.y + enemy.speed);
  const moveDown =
    enemy.difficulty === ENEMY_DIFFICULTY.OBSTACLE
      ? obstacleMoveDown
      : normalMoveDown;
  if (enemyIdx > -1) {
...
function playerShot(state: IAircraftWarContext) {
  ...
  if (
    ...
    now - bulletQueue[bulletQueue.length - 1].ct > playerShotRate
  ) {
    // 计算player头位置 减 5 为子弹固定长宽 10 / 2
    bulletQueue.push(
      generateBullet(playerX + playerW / 2 - 5, playerY, now, 'player')
    );
    return { ...state, bulletQueue: bulletQueue };
  } else {
    return state;
...
function bulletMove(state: IAircraftWarContext, id: string) {
  const {
    bulletQueue,
    gameBoundary: { up, down },
  } = state;
  const bullet = bulletQueue.find((b) => b.id === id);
  if (bullet) {
    bullet.y =
      bullet.belong === 'player'
...
function judgePlayerOverlapEnemy(state: IAircraftWarContext) {
  ...
  const aliveEnemys =
    enemyQueue.length > 0 ? enemyQueue.filter((v) => !v.isDestory) : [];
  let isHit = false;
  if (aliveEnemys.length > 0) {
    for (let i = 0; i < aliveEnemys.length; i++) {
      // 重叠被撞
      isHit = isOverlap(
        { x: playerX, y: playerY, w: playerW, h: playerH },
        aliveEnemys[i]
      );
      if (isHit) {
        break;
...
function judgeBulletOverlapPlayer(state: IAircraftWarContext) {
  ...
  if (existBullets.length > 0) {
    ...
    return isHit
      ? { ...state, bulletQueue: bulletQueue, gameStatus: GAME_STATUS.OVER }
      : state;
  } else {
    return state;
  }
...
function judgeBulletOverlapEnemy(state: IAircraftWarContext, enemyId: string) {
  ...
  const enemy =
    enemyQueue.length > 0 ? enemyQueue.find((v) => v.id === enemyId) : null;
  const existBullets =
    bulletQueue.length > 0
      ? bulletQueue.filter((v) => !v.isDestory && v.belong !== 'enemy')
      : [];
