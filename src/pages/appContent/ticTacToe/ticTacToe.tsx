...
const initBoardApi = {
  changeBoardValueByIndex: (
    rIndex: number,
    cIndex: number,
    value: ICheckerValueType
  ) => {
...
export default function TicTacToe() {
  ...
  const [checkerBoard, setCheckerBoard] =
    useState<ICheckerValueType[][]>(initBoard);
  const [emptyCheckerItems, setEmptyCheckerItems] = useState<
    [number, number][]
  >([[1, 1]]);
  ...
  const renderColItem = (value: string, rIndex: number, cIndex: number) => {
    return (
      <div
        ...
        key={`${rIndex}${cIndex}`}
        onClick={() => {
          playerClickColItem(rIndex, cIndex);
        }}
      >
        {value === 'null' ? '' : value}
      </div>
  ...
  /** 通过传入的坐标返回value */
  const getValueByIndex = (rIndex: number, cIndex: number) =>
    checkerBoard[rIndex][cIndex];
  ...
  const changeValueByIndex = (rIndex: number, cIndex: number) => {
    return new Promise((resolve) => {
      ...
      if (getValueByIndex(rIndex, cIndex) === 'null') {
        const newBoard = initBoardApi.changeBoardValueByIndex(
          rIndex,
          cIndex,
          targetValue
        );
  ...
  return (
    <div className="ticTacToeContainer">
      ...
      <div className={`overMessage textCenter ${gameover ? '' : 'none'}`}>
        <p className="mb10">
          {winner !== 'null'
            ? `${winner === 'player' ? '你' : '电脑'}赢了！`
            : '和局'}
        </p>
        <p className="pointer restartBtn" onClick={restartGame}>
          要不要再来一局
        </p>
      </div>
    </div>
